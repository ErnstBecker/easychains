...
def get_crypto_price(crypto_id, vs_currency):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Erro ao obter o preço da criptomoeda. Status code: {response.status_code}")
            return None
...
def get_price_history(crypto_id, vs_currency):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Erro ao obter o histórico de preços da criptomoeda. Status code: {response.status_code}")
            return None
    except requests.exceptions.RequestException as e:
        print(f"Erro de conexão: {e}")
        return None
...
def get_supported_cryptos():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Erro ao obter a lista de criptomoedas suportadas. Status code: {response.status_code}")
            return None
    except requests.exceptions.RequestException as e:
        print(f"Erro de conexão: {e}")
        return None
...
if __name__ == "__main__":
    ...
    layout = [[sg.Text('CriptoChecker', font=('Helvetica', 24), text_color='white')],
              [sg.Text('Por: Vítor Moura', font=(
                  'Helvetica', 10), text_color='white')],
              [sg.Text('Digite o nome da criptomoeda que deseja verificar:')],
              [sg.Input(key='crypto_id')],
              [sg.Text(
                  'Digite a moeda para converter (como por exemplo: "usd" ou "brl"):')],
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED:
            ...
            if price is not None:
                window['price_text'].update(
                    f"Preço da criptomoeda {crypto_id.upper()} em {vs_currency.upper()}: {price}")
                ...
                if price_history is not None:
                    ...
                    for timestamp, price in last_5_prices:
                        price_history_text.append(
                            f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(timestamp/1000))}: {price}")
                    window['price_history'].update(price_history_text)
                else:
                    window['price_text'].update(
                        "Não foi possível obter o histórico de preços da criptomoeda.")
            else:
                window['price_text'].update(
                    "Não foi possível obter o preço da criptomoeda.")
